<template>
  <div class="users">
    ...
    <!-- 按钮添加 -->
    <el-button class="addBtn" type="success" plain @click="dialogVisible = true">添加</el-button>
...
export default {
  ...
  methods: {
    ...
    async editMoney () {
      const { data } = await this.$axios.post(
        'hbte-financial/hbte/productLine/updateProductLine',
        this.editForm
      )
      if (data.code === 0) {
    ...
    async moneyadd () {
      ...
      // }
      const { data } = await this.$axios.post(
        'hbte-financial/hbte/productLine/saveProductLine',
        this.form
      )
      if (data.code === 0) {
