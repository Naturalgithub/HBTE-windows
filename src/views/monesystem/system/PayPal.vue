<template>
  <div class="users">
    ...
    <el-table :data="momeyManagementList" border style="width: 100%;margin-top:20px" size="mini">
      ...
      <el-table-column prop="paymentTermName" label="收款方式"/>
      ...
      <el-table-column label="操作">
        ...
        <template v-slot:default="obj">
          ...
          <el-button
            ...
            size="small"
            type="danger"
            @click="delUser(obj.row.paymentTermId)"
            icon="el-icon-delete"
          />
        </template>
      </el-table-column>
    ...
    />
    <!-- 模态框 -->
    <el-dialog title="收款方式管理" :visible.sync="dialogVisible" width="40%">
      <el-form ref="form" :model="form" status-icon label-width="100px">
        <el-form-item label="收款方式">
          <el-input v-model="form.paymentTermName" placeholder="请输入收款方式名称"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    showEditDialog (row) {
      ...
      console.log(row)
      // 回显基本的数据
      this.editForm.paymentTermName = row.paymentTermName
      this.editForm.currencyRatio = row.currencyRatio

      this.editForm.paymentTermId = row.paymentTermId
    },
    async delUser (id) {
      // console.log(id)
      // console.log(data)
      try {
        ...
        const { data } = await this.$axios.post(
          'hbte-financial/hbte/paymentTerm/delPaymentTerm',
          {
            paymentTermId: id
          }
        )
    ...
    async editMoney () {
      const { data } = await this.$axios.post(
        'hbte-financial/hbte/paymentTerm/updatePaymentTerm',
        this.editForm
      )
      if (data.code === 0) {
    ...
    async moneyadd () {
      ...
      // }
      const { data } = await this.$axios.post(
        'hbte-financial/hbte/paymentTerm/savePaymentTerm',
        this.form
      )
      if (data.code === 0) {
